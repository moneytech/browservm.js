...
bool process_interrupt_request(CPUArchState *env);
int process_exceptions(CPUArchState *env);
uintptr_t fetch_and_run_tb(TranslationBlock *prev_tb, int tb_exit_code, CPUArchState *env);
bool execution_loop(CPUArchState *env);
/*****************************************************************/
...
TranslationBlock *tb_find_slow(CPUArchState *env, target_ulong pc, target_ulong cs_base, uint64_t flags) {
...
#ifdef JIT_BUILD
        if (tb->tb_native_id) {
            prev_tb->jit_target_arg[tb_exit_code] = tb->tb_native_id;
        }
#endif
    }
...
#ifdef JIT_BUILD
    tb->tb_run_count++;
    if (!tb->tb_native_id && tb->tb_run_count >= JIT_THRESHOLD) {
        tcg_jit_translate_tb(env, tb);
    }
    
    if (!tb->tb_native_id) { /* Without Jit */
        last_tb = tcg_libcpu_tb_exec(env, tc_ptr);
    } else {
#ifdef __EMSCRIPTEN__
        last_tb = tcg_jit_execute_tb(tb->tb_native_id, env);
#else  /* Local debug */
        last_tb = tcg_libcpu_tb_exec(env, tc_ptr);
#endif
    }
...
bool execution_loop(CPUArchState *env) {
    ...
    for (;;) {
        ...
        if (env->kvm_request_interrupt_window && (env->mflags & IF_MASK)) {
            /* Ready to accept irq, get irq no from the main loop */
            return true;
        }

#ifdef __EMSCRIPTEN__
        hardware_poll_count ++;
        if (hardware_poll_count == 0) {
            poll_hardware(1);
        }
#endif
