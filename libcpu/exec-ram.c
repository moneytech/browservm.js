...
static void print_ram_blocks(void) {
    fprintf(stderr, "phys_dirty: %p  phys_dirty_size: %x\n", ram_list.phys_dirty, (uint32_t)ram_list.phys_dirty_size);

    for (unsigned i = 0; i < ram_list.block_count; ++i) {
        RAMBlock *block = &ram_list.blocks[i];
        fprintf(stderr, "host=%p offset=%#x length=%x\n", block->host, (uint32_t)block->offset, (uint32_t)block->length);
    }
}
